...
int main() {
    ...
    for (std::size_t i(0); i < num_lipid * 3; ++i)
        ...
        model->set_property(i, /* mass= */1.0e-10, /* friction= */1.0/tau);

    std::random_device rnd;
    std::mt19937 gen = std::mt19937(rnd());
    LangevinStepper stepper(space, model, gen, /* dt= */0.01*tau, /* T= */epsilon*1.2);

    SpaceXYZWriter writer("result_structure_unstable.xyz");
